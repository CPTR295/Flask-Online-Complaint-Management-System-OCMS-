...
class ComplaintDetailsRepository:
    ...
    def insert(self,cat:ComplaintDetails)->bool:
        try:
            self.sess.add(cat)
            self.sess.commit()
            return True
    ...
    def delete(self,id:int)->bool:
        try:
            self.sess.query(ComplaintDetails).filter(ComplaintDetails.id==id).delete()
            self.sess.commit()
            return True
        except Exception as e:
            print(e)
    ...
    def select_all(self)->List[Any]:
        cats = self.sess.query(ComplaintDetails).all()
        return cats
    
    def select_one(self,id:int)->Any:
        cat = self.sess.query(ComplaintDetails).filter(ComplaintDetails.id==id).one_or_none()
        return cat